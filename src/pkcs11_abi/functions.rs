...
pub extern "C" fn C_Sign(
    ...
    catch_unwind(|| with_hsm(|hsm_res| {
        ...
        // Fast path (perf/output-buffer-threading): for ECDSA/EdDSA, write
        // the signature directly into the caller's PKCS#11 output buffer to
        // eliminate the intermediate Vec allocation in `sign_single_shot`.
        // RSA mechanisms have no `_into_buf` variant — they fall through to
        // the legacy path below, which still benefits from the RSA handle
        // cache via the slot_id / key_handle args.
        ...
        match sign_single_shot_into_buf(mechanism, key_bytes, data, &obj, out_slice) {
            ...
            Ok(None) => {
                ...
                // legacy Vec<u8>-returning path below (covers RSA, PQC, etc.).
            }
        ...
        // Legacy path: pass slot_id + key_handle so the RSA backend can hit
        // RSA_PRIV_CACHE (perf/rsa-private-key-cache); other backends ignore
        // the extra args via the trait default impls.
        let slot_id = sess.slot_id;
